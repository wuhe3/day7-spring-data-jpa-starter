...
class CompanyControllerTest {
    ...
    void setUp() {
        ...
        john_smith = employeeRepository.save(new Employee(null, "John Smith", 32, Gender.MALE, 5000.0));
        jane_johnson = employeeRepository.save(new Employee(null, "Jane Johnson", 28, Gender.FEMALE, 6000.0));
        david_williams = employeeRepository.save(new Employee(null, "David Williams", 35, Gender.MALE, 5500.0));
        emily_brown = employeeRepository.save(new Employee(null, "Emily Brown", 23, Gender.FEMALE, 4500.0));
        michael_jones = employeeRepository.save(new Employee(null, "Michael Jones", 40, Gender.MALE, 7000.0));
        ...
        global_innovators = companyRepository.save(new Company(null, "Global Innovators", List.of()));
        stellar_enterprises = companyRepository.save(new Company(null, "Stellar Enterprises", List.of()));
        nexus_industries = companyRepository.save(new Company(null, "Nexus Industries", List.of()));
    }
    ...
    void should_return_paged_companies_when_get_by_page_params() throws Exception {
        ...
        client.perform(MockMvcRequestBuilders.get(String.format("/companies?pageIndex=%s&pageSize=%s", pageIndex + 1, pageSize)))
                ...
                .andExpect(MockMvcResultMatchers.jsonPath("$[0].name").value(nexus_industries.getName()));
    }
    ...
    void should_return_employees_when_get_employees_under_the_company() throws Exception {
        ...
        assertThat(fetchedEmployees)
                .usingRecursiveFieldByFieldElementComparator(
                        RecursiveComparisonConfiguration.builder()
                                .withComparedFields("name", "age", "gender", "salary")
                                .build()
                )
                .containsExactlyInAnyOrder(john_smith, jane_johnson);
    }
